...
// const elm = document.getElementById("graph");
...
function drawGraph(data) {
  ...
  function update(source) {
    ...
    //   if (targetId.includes(link.data.id)) {
    //     commmonTarget.push(index);
    //   } else {
    //     targetId.push(link.data.id);
    //   }
    // });
    // removedNode = [];
    ...
    //     .attr("class", link)
    //     .attr("d", function (d) {
    //       var o = { x: removedNode[0].x0, y: removedNode[0].y0 };
    //       return diagonal(o, o);
    //     });
    // }
    ...
    nodeEnter
      ...
      .attr("y", "-24")
      .attr("text-anchor", "start")
      .attr("class", "machine-name")
      .text(function (d) {
        return "(" + d.data.additional_data.current_machine.trim() + ")";
      })
      .call(shiftText, 40);
    ...
    nodeEnter
      ...
      .attr("xlink:href", function (v) {
        if (v.data.type == "machine") {
          var type = v.data.additional_data.os
            ? "-" + v.data.additional_data.os.toLowerCase()
            : "";
          type += v.data._children && v.data._children.length ? "-expand" : "";
          if (v.data.type == "machine" && !v.data.additional_data.has_access)
            type = "-noaccess";
          if (v.data.is_unknown) type += "-unknown";
          return getIcon(v.data.type, type);
        } else if (v.data.type == "application") {
          var type = "-all";
          return getIcon(v.data.type, type);
        } else if (v.data.type == "appstack") {
          var type = "-all";
          return getIcon(v.data.type, type);
        }
      });
    ...
    // if (removedNode.length && link._groups[0].includes(undefined)) {
    ...
    d3.select("#graph .node").on("contextmenu", function (data, index) {
      debugger;
  ...
  function shiftText(text, diameter) {
    text._groups[0].forEach(function (nameElem) {
      if (nameElem != null) {
        var textWidth = nameElem.getBoundingClientRect().width;
        var x = 0;
        if (diameter < textWidth) {
          x = (diameter - textWidth - 40) / 2;
        } else if (diameter > textWidth) {
          x = (diameter - textWidth - 40) / 2;
        }
